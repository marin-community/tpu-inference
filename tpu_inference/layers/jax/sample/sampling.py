...
class PromptLogprobsAsyncData:
    """Holds async-copied prompt logprob tensors and request snapshots."""
...
def compute_and_gather_prompt_logprobs(
    ...
    """Compute logprobs and gather top-k entries for prompt tokens."""
    prompt_target_ids = jnp.roll(input_ids, -1, axis=0)
    return compute_and_gather_logprobs(logits, prompt_target_ids,
                                       max_logprobs)
...
def compute_prompt_logprobs(
    ...
    """Compute prompt logprobs and snapshot per-request slicing metadata."""
